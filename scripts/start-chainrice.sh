#!/bin/bash
# ChainRice Development Environment Startup Script
# Starts only ChainRice frontend with blockchain and smart contracts
...
print_status "⛓️  Starting blockchain service..."
if [ -f "libs/backend/modules/blockchain/scripts/start-blockchain.sh" ]; then
    ...
    print_success "Blockchain service started"
else
    print_warning "Blockchain startup script not found, simulating blockchain..."
    # Simulate blockchain startup
    (
...
# Start smart contracts service
print_status "📜 Starting smart contracts service..."
if [ -f "libs/contract/solidity/package.json" ]; then
    cd libs/contract/solidity
    npx hardhat node --hostname 0.0.0.0 --port 8545 > /tmp/hardhat.log 2>&1 &
    echo $! > /tmp/hardhat.pid
    cd ../../..
    print_success "Hardhat node started on http://localhost:8545"
else
    print_warning "Solidity contracts not found, simulating smart contracts..."
    (
...
if command -v xdg-open > /dev/null; then
    ...
    print_info "Dashboard available at: http://localhost:8080/chainrice-dashboard.html"
fi

# Open ChainRice frontend in browser
if command -v xdg-open > /dev/null; then
