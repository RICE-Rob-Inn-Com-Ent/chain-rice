...
pub mod warden {
//! Thin Rust bindings for Zig [`warden_*`](../../security/src/warden.zig) exports (`clerk-security` static lib).

#[link(name = "clerk-security", kind = "static")]
unsafe extern "C" {
    fn warden_check_leverage_limit(
        principal_len: usize,
        max_principal_len: usize,
        bps: i64,
        max_bps: i64,
    ) -> i32;
    fn warden_prune_memory_remnants(ptr: *mut u8, len: usize);
    fn warden_secure_wipe(ptr: *mut u8, len: usize);
}
...
pub fn check_leverage_limit(
    ...
    unsafe { warden_check_leverage_limit(principal_len, max_principal_len, bps, max_bps) == 0 }
}
...
pub unsafe fn prune_memory_remnants(ptr: *mut u8, len: usize) {
    ...
    unsafe { warden_prune_memory_remnants(ptr, len) };
}
...
pub unsafe fn secure_wipe(ptr: *mut u8, len: usize) {
    ...
    unsafe { warden_secure_wipe(ptr, len) };
}
...
#[cfg(all(feature = "zig-warden", unix))]
pub mod stress {
//! Sovereign stress suite — C ABI [`security_stress_test`](../../security/src/chaos.zig) from `clerk-security`.

#[link(name = "clerk-security", kind = "static")]
unsafe extern "C" {
    fn security_stress_test() -> i32;
}
...
//! # Wire format
//!
//! ## Fast path (binary v1) — when `CLERK_CALC_USE_BINARY=1`
//!
...
//! # Runtime
//!
//! Load the shared library from **`CLERK_CALC_LIB`**. `calc_init` runs once per loaded library; [`shutdown`]
//! calls `calc_shutdown` then `dlclose`.
//!
//! With **`zig-warden`** (in addition to **`calc-bridge`**), Zig `warden_*` runs input caps and
...
const CLERK_BPS_BIN_MAGIC_V1: u32 = 1;
const CLERK_BPS_BIN_MAX_PRINCIPAL: usize = 4096;

/// Zig warden: reject out-of-range inputs before crossing into Haskell RTS (defense-in-depth).
#[cfg(feature = "zig-warden")]
fn warden_guard_calc_input(principal_minor_units: &str, basis_points: i64) -> Result<()> {
    const MAX_BPS: i64 = 1_000_000;
    if !warden::check_leverage_limit(
        principal_minor_units.len(),
        CLERK_BPS_BIN_MAX_PRINCIPAL,
        basis_points,
        MAX_BPS,
    ) {
        return Err(Error::finance(String::from(
...
fn with_inner<T>(f: impl FnOnce(&Library) -> Result<T>) -> Result<T> {
    let mut slot = CALC.lock().map_err(|_| Error::message("calc-bridge: state mutex poisoned"))?;
    if slot.is_none() {
        let path = std::env::var_os("CLERK_CALC_LIB").ok_or_else(|| {
            Error::message(
                "calc-bridge: set CLERK_CALC_LIB to the path of libcalc_ffi.so (from `cabal build foreign-library:calc_ffi`)",
            )
        })?;
        let lib = unsafe { Library::new(path.as_os_str()) }.map_err(|e| Error::message(format!("calc-bridge dlopen: {e}")))?;
        *slot = Some(Inner {
            lib,
            rts_inited: false,
        });
    ...
    if !inner.rts_inited {
        ...
        let init: Symbol<InitFn> = unsafe { inner.lib.get(b"calc_init\0") }.map_err(|e| {
            Error::message(format!("calc-bridge: symbol calc_init: {e}"))
        })?;
        unsafe { init() };
...
pub fn shutdown() -> Result<()> {
    ...
    if let Some(inner) = slot.take() {
        ...
        if let Ok(sym) = unsafe { inner.lib.get::<ShutdownFn>(b"calc_shutdown\0") } {
            unsafe { sym() };
        }
        drop(inner.lib);
    }
...
fn pack_bps_binary_v1(principal_minor_units: &str, basis_points: i64) -> Result<Vec<u8>> {
    let pb = principal_minor_units.as_bytes();
    let plen = pb.len();
    if plen > CLERK_BPS_BIN_MAX_PRINCIPAL {
        return Err(Error::message(format!(
            "calc-bridge binary v1: principal length {plen} exceeds {CLERK_BPS_BIN_MAX_PRINCIPAL}"
        )));
    }
    let mut v = Vec::with_capacity(16 + plen);
    v.extend_from_slice(&CLERK_BPS_BIN_MAGIC_V1.to_le_bytes());
...
fn parse_bps_binary_response_v1(slice: &[u8]) -> Result<String> {
    if slice.len() < 8 {
        return Err(Error::message(format!(
    ...
    if magic != CLERK_BPS_BIN_MAGIC_V1 {
        return Err(Error::message(format!("calc-bridge binary v1: bad magic {magic}")));
    }
    let fee_len = u32::from_le_bytes(slice[4..8].try_into().expect("len >= 8")) as usize;
    if slice.len() < 8 + fee_len {
        return Err(Error::message("calc-bridge binary v1: truncated fee"));
    }
    let fee = std::str::from_utf8(&slice[8..8 + fee_len]).map_err(|e| Error::message(format!("calc-bridge binary v1: fee UTF-8: {e}")))?;
    Ok(fee.to_string())
}

fn call_calc_fee_json(lib: &Library, principal_minor_units: &str, basis_points: i64) -> Result<String> {
    type FeeJsonFn = unsafe extern "C" fn(*const u8, size_t, *mut *mut u8, *mut size_t) -> c_int;
    type FreeFn = unsafe extern "C" fn(*mut u8);

    let fee_json: Symbol<FeeJsonFn> = unsafe { lib.get(b"calc_amount_from_basis_points_json\0") }.map_err(|e| {
        Error::message(format!("calc-bridge: symbol calc_amount_from_basis_points_json: {e}"))
    })?;
    let free: Symbol<FreeFn> =
        unsafe { lib.get(b"calc_free\0") }.map_err(|e| Error::message(format!("calc-bridge: symbol calc_free: {e}")))?;

    let body = json!({
        "principal_minor_units": principal_minor_units,
        "basis_points": basis_points,
    });
    let input = serde_json::to_vec(&body).map_err(|e| Error::message(format!("calc-bridge encode: {e}")))?;
    ...
    unsafe {
        ...
        if rc != 0 {
            ...
            return Err(Error::finance(format!(
                "calc-bridge calc_amount_from_basis_points_json rc={rc}"
            )));
        }
        if out_ptr.is_null() || out_len == 0 {
            return Err(Error::finance(String::from(
                "calc-bridge: empty response from calc_amount_from_basis_points_json",
            )));
        }
        ...
        let parsed: BasisPointsFeeResponse = serde_json::from_slice(slice).map_err(|e| {
            ...
            warden::prune_memory_remnants(out_ptr, out_len);
            free(out_ptr);
            Error::message(format!("calc-bridge decode: {e}"))
        })?;
        #[cfg(feature = "zig-warden")]
        warden::prune_memory_remnants(out_ptr, out_len);
        free(out_ptr);
...
fn call_calc_fee_binary(lib: &Library, principal_minor_units: &str, basis_points: i64) -> Result<String> {
    type FeeBinFn = unsafe extern "C" fn(*const u8, size_t, *mut *mut u8, *mut size_t) -> c_int;
    type FreeFn = unsafe extern "C" fn(*mut u8);

    let fee_bin: Symbol<FeeBinFn> = unsafe { lib.get(b"calc_basis_points_fee_binary\0") }.map_err(|e| {
        Error::message(format!("calc-bridge: symbol calc_basis_points_fee_binary: {e}"))
    })?;
    let free: Symbol<FreeFn> =
        unsafe { lib.get(b"calc_free\0") }.map_err(|e| Error::message(format!("calc-bridge: symbol calc_free: {e}")))?;
    ...
    unsafe {
        ...
        if rc != 0 {
            ...
            return Err(Error::finance(format!(
                "calc-bridge calc_basis_points_fee_binary rc={rc}"
            )));
        }
        if out_ptr.is_null() || out_len == 0 {
            return Err(Error::finance(String::from(
                "calc-bridge: empty response from calc_basis_points_fee_binary",
            )));
        }
        let slice = std::slice::from_raw_parts(out_ptr, out_len);
...
///
/// When **`CLERK_CALC_USE_BINARY=1`**, uses the binary v1 ABI (`calc_basis_points_fee_binary`).
/// Otherwise uses JSON (`calc_amount_from_basis_points_json`).
pub fn basis_points_fee_minor_units(principal_minor_units: &str, basis_points: i64) -> Result<String> {
    with_inner(|lib| {
        if std::env::var("CLERK_CALC_USE_BINARY").ok().as_deref() == Some("1") {
...
#[cfg(test)]
mod tests {
    use super::{pack_bps_binary_v1, parse_bps_binary_response_v1, CLERK_BPS_BIN_MAGIC_V1};

    #[test]
    fn binary_v1_pack_len_matches_layout() {
        let v = pack_bps_binary_v1("10000", 250).unwrap();
        assert_eq!(v.len(), 16 + 5);
        assert_eq!(u32::from_le_bytes(v[0..4].try_into().unwrap()), CLERK_BPS_BIN_MAGIC_V1);
    }

    #[test]
    fn binary_v1_parse_response_matches_haskell_layout() {
        let mut out = Vec::new();
        out.extend_from_slice(&CLERK_BPS_BIN_MAGIC_V1.to_le_bytes());
        let fee = b"250";
