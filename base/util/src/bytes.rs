...
/// surface as [`Error::Hex`] for uniform `?` handling across `base/`.
#[inline]
...
#[inline]
pub fn bytes_from_hex(s: &str) -> Result<Bytes> {
...
#[inline]
pub fn bytes_slice(buf: &Bytes, start: usize, end: usize) -> Result<Bytes> {
...
#[inline]
pub fn fixed_size_array<const N: usize>(data: &[u8]) -> Result<[u8; N]> {
    if data.len() != N {
        return Err(Error::invalid_argument(format!(
...
fn reject_zero_width(name: &str, width: usize) -> Result<()> {
    if width == 0 {
        return Err(Error::invalid_argument(format!(
...
/// places `pad_byte` on the chosen [`PadSide`]. If longer, returns [`Error::InvalidArgument`]
...
pub fn pad_to_exact_length(
    ...
    match data.len().cmp(&target_len) {
        ...
        std::cmp::Ordering::Greater => Err(Error::invalid_argument(format!(
...
pub fn pad_to_multiple_of(
    ...
    let target_len = len
        .checked_add(pad_len)
        .ok_or_else(|| Error::invalid_argument("padding length overflow"))?;
...
#[inline]
pub fn take_from_buf<B: Buf>(buf: &mut B, n: usize) -> Result<Bytes> {
    if buf.remaining() < n {
        return Err(Error::invalid_argument(format!(
...
/// storage as `buf`.
#[inline]
pub fn bytes_split_to_front(buf: &mut Bytes, n: usize) -> Result<Bytes> {
    if buf.len() < n {
        return Err(Error::invalid_argument(format!(
...
#[inline]
pub fn try_put_slice_without_realloc<B: BufMut>(buf: &mut B, data: &[u8]) -> Result<()> {
    let need = data.len();
...
/// Advance `buf` by `n` bytes, mapping underflow to [`Error`].
///
...
#[inline]
pub fn advance_buf<B: Buf>(buf: &mut B, n: usize) -> Result<()> {
    if buf.remaining() < n {
        return Err(Error::invalid_argument(format!(
