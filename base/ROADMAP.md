...
| Perf harness ↔ bench | Done | `CLERK_BENCH_BIN`, `CLERK_LSP_PERF` |

## `policies` — CEL, finance, audit

| Item | Status | Notes |
|------|--------|--------|
| CEL engine, finance, XML/FIX paths | Done | Optional `CLERK_*` env in [`.env`](.env) |
| CALC delegation (`CLERK_USE_CALC_FFI`) | Next | Wire to `util::calc` in hot paths consistently |
...
| Item | Status | Notes |
|------|--------|--------|
| Groth16 pipeline, env-driven setup | Done | `CLERK_ZK_*` in [`.env`](.env) |
| Production trusted setup + HSM story | Next | `CLERK_ZK_TRUSTED_SETUP_PATH` workflow |
...
| Item | Status | Notes |
|------|--------|--------|
| Stable error surface (`util::Error`) | Done | |
