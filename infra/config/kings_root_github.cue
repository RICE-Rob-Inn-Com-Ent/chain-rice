...
// =============================================================================
// KING .github (SINGLE SOURCE FOR ROOT .github FOLDER)
// =============================================================================
//
// This file is the single source of truth for the repository root .github/
// folder: issue templates, workflows, CODEOWNERS, FUNDING.yml, PR template, etc.
// Manifest (infra/manifest.cue) uses king_folder_file_outputs to write these files.
// =============================================================================

_bug_report_yml: """
	name: 🐛 Bug Report
	description: Report a bug or unexpected behavior in Rice Monorepo
	title: "[BUG] "
	    ...
	    attributes:
	      value: |
	        ...
	        **Before submitting**, please:
	        ...
	        - ✅ Check the [documentation](../../tree/main/.doc/docs) for solutions
...
_feature_request_yml: """
	name: ✨ Feature Request
	description: Suggest a new feature or enhancement for Rice Monorepo
	title: "[FEATURE] "
	    ...
	    attributes:
	      value: |
	        ...
	        **Before submitting**, please:
	        ...
	        - ✅ Check the [roadmap](../../discussions/categories/roadmap)
	        - ✅ Review [documentation](../../tree/main/.doc/docs)
...
_question_yml: """
	...
	description: Ask a question or start a discussion about Rice Monorepo
	title: "[QUESTION] "
	labels: ["question"]
	assignees:
	  - mrDinkelman
	  ...
	  - type: markdown
	    attributes:
	      value: |
	        ...
	        Use this template for questions, discussions, advice, or documentation clarification.
	        **Before asking**, please check the [documentation](../../tree/main/.doc/docs) and [discussions](../../discussions).
	  ...
	  - type: dropdown
	    ...
	    attributes:
	      ...
	      options:
	        ...
	        - Development - How to build/develop
...
_workflow_cd_project: """
	name: Auto-Sync Project Repository

	on:
	  push:
	    branches: [v.0.1.0, main]
	    paths:
	      - ".project/**"
	  workflow_dispatch:
	...
	jobs:
	  sync-project:
	    name: Sync .project to Target Repository
	    ...
	    steps:
	      ...
	      - name: Show project metadata
	        run: |
	          echo "📦 Project:   ${{ steps.context.outputs.active_project_name }} (${{ steps.context.outputs.active_project }})"
	          echo "📁 Directory: ${{ steps.context.outputs.project_path }}"
	          echo "🔗 Repo URL:  ${{ steps.context.outputs.repo_url }}"

	      - name: Clone Target Repository
	        run: |
	          REPO_URL="${{ steps.context.outputs.repo_url }}"
	          BRANCH="${{ steps.context.outputs.repo_branch }}"
	          git clone $REPO_URL /tmp/target-repo
	          cd /tmp/target-repo
	          git checkout $BRANCH 2>/dev/null || git checkout -b $BRANCH

	      - name: Sync project directory to target repository
	        run: |
	  ...
	  secret-scan:
	    ...
	    steps:
	      ...
	      - name: Install scanners
	        run: |
	          python3 -m pip install --upgrade pip detect-secrets
	          curl -sSLo gitleaks.tgz https://github.com/gitleaks/gitleaks/releases/latest/download/gitleaks-linux-amd64.tar.gz
	  ...
	  sonarcloud:
	    ...
	    steps:
	      ...
	      - name: SonarCloud Scan
	        ...
	        with:
	          args: >
	            -Dsonar.projectKey=rice-mono
	            -Dsonar.organization=rice-dev
	...
	on:
	  push:
	    branches:
	      ...
	      - develop
	    paths:
	      - ".devcontainer/auto/terraform/**"
	      - ".devcontainer/auto/kubernetes/**"
	...
	jobs:
	  deploy:
	    ...
	    steps:
	      ...
	      - name: Terraform Init
	        working-directory: .devcontainer/auto/terraform
	        run: terraform init
	      ...
	      - name: Deploy Serverless Stack
	        working-directory: .devcontainer/auto/kubernetes
	        run: |
	          ...
	          helm upgrade --install rice-mono . \\
	            ...
	            --set serverless.clusterAutoscaler.cloudProvider=azure \\
	            --set serverless.clusterAutoscaler.azure.resourceGroup=${{ secrets.AKS_RESOURCE_GROUP }} \\
	            --set serverless.knative.enabled=true \\
	            --set serverless.keda.enabled=true

	      - name: Health Check
	        run: |
	          kubectl wait --for=condition=ready pod -l app=postgresql-cluster -n rice-mono --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=knative-serving -n knative-serving --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=keda-operator -n keda --timeout=300s || true
	"""

_workflow_deploy_eks: """
	  ...
	  deploy:
	    ...
	    steps:
	      ...
	      - name: Deploy Serverless Stack
	        ...
	        run: |
	          ...
	          helm upgrade --install rice-mono . \\
	            ...
	            --set serverless.enabled=true \\
	            --set serverless.clusterAutoscaler.enabled=true \\
	            --set serverless.clusterAutoscaler.cloudProvider=aws \\
	            --set serverless.clusterAutoscaler.autoDiscovery.clusterName=${{ secrets.EKS_CLUSTER_NAME }} \\
	            --set serverless.clusterAutoscaler.aws.region=${{ env.AWS_REGION }} \\
	            --set serverless.knative.enabled=true \\
	            --set serverless.keda.enabled=true
	      ...
	      - name: Health Check
	        run: |
	          kubectl wait --for=condition=ready pod -l app=postgresql-cluster -n rice-mono --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=mongodb-replica-set -n rice-mono --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=cluster-autoscaler -n kube-system --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=knative-serving -n knative-serving --timeout=300s || true
	          kubectl wait --for=condition=ready pod -l app=keda-operator -n keda --timeout=300s || true
	"""

_workflow_deploy_gke: """
...
// Pełna treść workflowów (z oryginalnych plików infra/github/workflows).
_workflow_ci_typecheck: """
	name: Type Check
	...
	jobs:
	    project-metadata:
	        name: Discover Frontend Projects
	        runs-on: ubuntu-latest
	        outputs:
	            active_project: ${{ steps.context.outputs.active_project }}
	            active_project_name: ${{ steps.context.outputs.active_project_name }}
	            frontends: ${{ steps.context.outputs.frontends }}
	            bun_projects: ${{ steps.context.outputs.bun_projects }}
	        steps:
	    ...
	    typescript-typecheck:
	        ...
	        steps:
	            ...
	            - name: Run TypeScript compiler for frontend projects
	              env:
	                  FRONTENDS: ${{ needs.project-metadata.outputs.frontends }}
	              run: |
	                  set -eo pipefail
	                  FRONTENDS="${FRONTENDS:-[]}"
	                  if [ "$FRONTENDS" = "[]" ]; then
	                  ...
	                  echo "$FRONTENDS" | jq -c '.[]' | while read -r project; do
	                    ...
	                    echo "::group::Type checking $name ($path)"
	                    if [ ! -d "$path" ]; then
	                      echo "Skipping $name (missing $path)"
	                      echo "::endgroup::"
	                      continue
	                    fi
	                    (cd "$path" && bun install --frozen-lockfile && bun run tsc --noEmit 2>/dev/null || bunx tsc --noEmit --pretty 2>/dev/null || true)
	                    (cd "$path" && bunx type-coverage --at-least 90 2>/dev/null) || true
	                    echo "::endgroup::"
	                  done
	    ...
	    python-typecheck:
	        ...
	        steps:
	            ...
	            - name: Run mypy
	              working-directory: .bot
	              run: |
	                  poetry run mypy core integration \\
	                    --strict \\
	                    --pretty \\
	                    --show-error-codes \\
	                    --show-column-numbers \\
	            ...
	            - name: Run pyright
	              ...
	              run: |
	                  ...
	                  poetry run pyright core integration
	              continue-on-error: true
	    ...
	    go-typecheck:
	        ...
	        steps:
	            ...
	            - name: Set up Go
	              ...
	              with:
	                  go-version: "1.22"
	                  cache-dependency-path: .backend/${{ matrix.component }}/go.sum
	    ...
	    rust-typecheck:
	        ...
	        steps:
	            ...
	            - name: Cache Rust dependencies
	              ...
	              with:
	                  workspaces: .devcontainer/contract

	            - name: Run cargo check
	              working-directory: .devcontainer/contract
	        ...
	        steps:
	            - name: Generate summary
	              run: |
	                  ...
	                  echo "|----------|--------|" >> $GITHUB_STEP_SUMMARY
	                  echo "| TypeScript | ${{ needs.typescript-typecheck.result }} |" >> $GITHUB_STEP_SUMMARY
	                  echo "| Python | ${{ needs.python-typecheck.result }} |" >> $GITHUB_STEP_SUMMARY
	                  echo "| Go | ${{ needs.go-typecheck.result }} |" >> $GITHUB_STEP_SUMMARY
	                  echo "| Rust | ${{ needs.rust-typecheck.result }} |" >> $GITHUB_STEP_SUMMARY

	                  if [ "${{ needs.typescript-typecheck.result }}" != "success" ] || \\
	                     [ "${{ needs.python-typecheck.result }}" != "success" ] || \\
	                     [ "${{ needs.go-typecheck.result }}" != "success" ] || \\
	                     [ "${{ needs.rust-typecheck.result }}" != "success" ]; then
	                    echo "" >> $GITHUB_STEP_SUMMARY
	                    ...
	                    echo "" >> $GITHUB_STEP_SUMMARY
	                    echo "### Active Project" >> $GITHUB_STEP_SUMMARY
	                    echo "- Name: **${{ needs.project-metadata.outputs.active_project_name || needs.project-metadata.outputs.active_project || 'n/a' }}**" >> $GITHUB_STEP_SUMMARY
	                  fi
	"""

_workflow_cd_release: ""
_workflow_ci_format:  ""
_workflow_ci_lint:    ""
_workflow_ci_test:    ""

// -----------------------------------------------------------------------------
// .github file map and outputs for manifest
// -----------------------------------------------------------------------------
github_files: {
	"CODEOWNERS": """
...
// king_folder_file_outputs: list of {path, content} for manifest to write (root .github/ from CUE).
