...
```text
.backend/          → Backend services & shared infrastructure runtime
.project/          → Active client projects (mounted into tooling)
.proto/            → Protocol Buffer definitions & API schemas
.devcontainer/     → Development environment, pipelines, IaC tooling
.document/         → All documentation and reference material
.github/           → CI/CD workflows & shared GitHub automation
.snyk/             → Security scanning rules & baselines
```

**Philosophy**:

- **Dot-prefix folders**: First-class components (`.backend/`, `.project/`)
- **No prefix**: Root configuration files
- **Clear separation**: Each folder has single responsibility
- **Scalable**: Easy to add new components

> ℹ️ Frontend workspaces now live inside `.project/<client>/web` and are generated by devcontainer automation. The legacy `.frontend/` directory has been retired.
...
### `.devcontainer/` - Infrastructure as Code & Tooling
...
```text
.devcontainer/
├── auto/                 # IaC & lifecycle (workspace 🏗️ INFRA)
│   ├── app/              # Ansible (hosts.yml, playbooks at root, tasks/*.yml, templates/, roles/geerlingguy.docker)
│   ├── terraform/        # Cloud infrastructure (AWS/GCP/Azure)
│   ├── kubernetes/       # Helm chart + manifests
│   ├── setup/            # Optional hooks (pre-config-extra, <slug>.sh, post-config)
│   └── unix/             # Bash automation (pre-config, configure, post-config)
├── backend/              # Go services
├── bot/                  # Python AI integrations
├── contract/             # Smart contracts
├── devcontainer.json     # Devcontainer config
└── README.md             # Devcontainer overview
```
...
```bash
# Terraform
cd .devcontainer/auto/terraform
terraform init
terraform plan
terraform apply

# Kubernetes
cd .devcontainer/auto/kubernetes
helm lint .
helm template .
kubectl apply -f templates/

# Ansible
cd .devcontainer/auto
...
2. Cloud: `.devcontainer/auto/terraform/`
3. K8s: `.devcontainer/auto/kubernetes/templates/`
