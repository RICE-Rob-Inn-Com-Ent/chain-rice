...
import { Invoice, DashboardStats, Category, ReceiptProcessResponse } from '../types/accounting';
...
export const invoicesApi = {
  list: async (params?: {
    ...
    start_date?: string;
    end_date?: string;
  }): Promise<{ invoices: Invoice[]; total_count: number; page: number; page_size: number }> => {
...
export const aiApi = {
  ...
  processAndSaveReceipt: async (file: File): Promise<ReceiptProcessResponse> => {
    const formData = new FormData();
    formData.append('file', file);
    
    const response = await aiApiClient.post('/process-receipt-and-save', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  uploadFile: async (file: File): Promise<{ file_path: string; file_id: string }> => {
    const formData = new FormData();
    formData.append('file', file);
