...
export default async function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  if (role !== "owner") {
    const rolePrefix = role === "admin" ? "adm" : role === "manager" ? "mgr" : role === "developer" ? "dev" : "own";
    redirect(`/${user.id}`);
  }
      ...
      <aside className="hidden lg:block w-64 border-r border-white/10 bg-void-900/50">
        <div className="flex h-full flex-col">
          <div className="border-b border-white/10 p-6">
            <h1 className="font-display text-2xl text-white">RICE Owner</h1>
            <p className="mt-1 text-sm text-white/60">{user.display_name}</p>
            <p className="mt-1 text-xs text-white/40 uppercase">Właściciel</p>
          </div>
          <nav className="flex-1 space-y-1 p-4">
            ...
            <Link
              ...
              className="flex items-center gap-3 rounded-lg px-4 py-3 text-white/70 transition hover:bg-white/5 hover:text-white"
            >
              <span className="material-symbols-outlined text-[1.5rem]">people</span>
            ...
            <Link
              href={`/${user.id}/infrastructure`}
              className="flex items-center gap-3 rounded-lg px-4 py-3 text-white/70 transition hover:bg-white/5 hover:text-white"
            >
              <span className="material-symbols-outlined text-[1.5rem]">cloud</span>
            ...
            <Link
              ...
              className="flex items-center gap-3 rounded-lg px-4 py-3 text-white/70 transition hover:bg-white/5 hover:text-white"
            >
              <span className="material-symbols-outlined text-[1.5rem]">settings</span>
