...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (role !== "admin") {
    const rolePrefix = role === "owner" ? "own" : role === "manager" ? "mgr" : role === "developer" ? "dev" : "adm";
    redirect(`/${user.id}`);
  }
      ...
      <aside className="hidden lg:block w-64 border-r border-white/10 bg-void-900/50">
        <div className="flex h-full flex-col">
          <div className="border-b border-white/10 p-6">
            <h1 className="font-display text-2xl text-white">RICE Admin</h1>
            <p className="mt-1 text-sm text-white/60">{user.display_name}</p>
            <p className="mt-1 text-xs text-white/40 uppercase">Administrator</p>
          </div>
