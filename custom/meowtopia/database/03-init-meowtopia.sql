-- Meowtopia Database Initialization
-- Tworzy bazę danych meowtopia z przykładowymi użytkownikami
-- This script runs after the database is created

-- Połącz się z bazą meowtopia przed uruchomieniem reszty:
\c meowtopia

-- Grant schema privileges to meowtopia_user
GRANT ALL ON SCHEMA public TO meowtopia_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO meowtopia_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO meowtopia_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON FUNCTIONS TO meowtopia_user;
...
FROM users WHERE email = 'superadmin@meowtopia.com'
...
FROM users WHERE email = 'admin@meowtopia.com'
ON CONFLICT (user_id, role) DO NOTHING;
...
INSERT INTO user_roles (user_id, role, granted_at)
SELECT id, 'manager', CURRENT_TIMESTAMP
FROM users WHERE email = 'manager@meowtopia.com'
ON CONFLICT (user_id, role) DO NOTHING;
...
FROM users WHERE email = 'user@meowtopia.com'
ON CONFLICT (user_id, role) DO NOTHING;
