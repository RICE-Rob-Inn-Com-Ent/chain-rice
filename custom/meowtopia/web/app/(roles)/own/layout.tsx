...
export default async function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  if (role !== "OWNER") {
    const rolePrefix = role === "ADMIN" ? "adm" : role === "MANAGER" ? "mgr" : role === "CUSTOMER" ? "cus" : "own";
    redirect(`/${user.id}`);
  }
      ...
      <aside className="hidden lg:block w-64 border-r border-white/10 bg-jungle-900/50">
        <div className="flex h-full flex-col">
          <div className="border-b border-white/10 p-6">
            <h1 className="font-display text-2xl text-foam-50">MeoWTopia Owner</h1>
            <p className="mt-1 text-sm text-foam-100/60">{user.name || user.email}</p>
            <p className="mt-1 text-xs text-foam-100/40 uppercase">Właściciel</p>
          </div>
