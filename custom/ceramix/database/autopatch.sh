#!/bin/sh
# Autopatcher for ceramix-database (Go service)
# Monitors Go source files and triggers rebuild/restart

set -e

WATCH_DIR="/app"
CONFIG_FILES="/app/go.mod /app/go.sum /app/main.go"
...
check_and_restart() {
    ...
    echo "   Container will restart on next check..."
    kill 1 || exit 1
}
