...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!accessToken) {
      return NextResponse.redirect(`${baseUrl}/sign-in?error=no_access_token`);
    }
    ...
    if (!email) {
      return NextResponse.redirect(`${baseUrl}/sign-in?error=no_email`);
    }

    // Find or create user
    let user = await findOAuthAccount("github", userInfo.id.toString());
    ...
    if (!user) {
      ...
      await query(
        `UPDATE oauth_accounts SET display_name = $1, avatar_url = $2 WHERE provider = 'github' AND provider_id = $3`,
        [userInfo.name || userInfo.login, userInfo.avatar_url, userInfo.id.toString()]
      );
    }
