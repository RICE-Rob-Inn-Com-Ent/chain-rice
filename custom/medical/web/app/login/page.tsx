  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.requires2FA) {
        ...
        // Always use redirect from API response, it knows the user's role
        let redirectUrl = data.redirect || "/account";
        
        // After successful login, ALWAYS redirect to panel subdomain (standardized)
        if (data.usePanelDomain && typeof window !== 'undefined') {
          ...
          redirectUrl = `${protocol}//${panelDomain}${redirectUrl}`;
  ...
  const handle2FA = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log(`[login 2FA] Redirecting to panel: ${redirectUrl}`);
        // Use window.location.href for full page redirect (including domain change)
        window.location.href = redirectUrl;
        return;
      }
