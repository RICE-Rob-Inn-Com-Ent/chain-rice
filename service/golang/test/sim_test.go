...
func BenchmarkSimulation(b *testing.B) {
	...
	appOptions[flags.FlagHome] = token.DefaultNodeHome

	bApp := token.New(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
	require.Equal(b, token.Name, bApp.Name())
...
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	appOptions.SetDefault(flags.FlagHome, token.DefaultNodeHome)

	app := token.New(logger, db, nil, true, appOptions, interBlockCacheOpt(), baseapp.SetChainID(SimAppChainID))
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := token.New(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func TestAppImportExport(t *testing.T) {
	...
	bApp := token.New(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
	require.Equal(t, token.Name, bApp.Name())
	...
	newApp := token.New(log.NewNopLogger(), newDB, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
	require.Equal(t, token.Name, newApp.Name())
	...
	// Convert GenesisState to map[string]json.RawMessage format expected by InitGenesis
	genesisBytes, err := json.Marshal(genesisState)
	require.NoError(t, err)
	genesisMap := map[string]json.RawMessage{
		"token": genesisBytes,
	}
	_, err = newApp.ModuleManager.InitGenesis(ctxB, bApp.AppCodec(), genesisMap)
